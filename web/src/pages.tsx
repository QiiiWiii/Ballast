...
import { EmptyState, ErrorPanel, Metric, PageHeader, PanelTitle, RelativeTime, StabilityLine, statusText, StatusPill, TaskProgress, currentVersion } from "./components";
...
export function ApprovalsPage(){
  const {t}=useTranslation(); const client=useQueryClient(); const approvals=useQuery({queryKey:["approvals"],queryFn:api.approvals});
  const approve=useMutation({mutationFn:api.approveTask,onSuccess:async()=>client.invalidateQueries({queryKey:["approvals"]})});
  const reject=useMutation({mutationFn:(id:string)=>api.rejectTask(id,"rejected_by_operator"),onSuccess:async()=>client.invalidateQueries({queryKey:["approvals"]})});
  return <><PageHeader eyebrow="LIVE / APPROVAL" title={t("approvals")} description={t("privatePlaneDescription")}/><div className="mode-banner locked-banner"><b>FAIL CLOSED</b><span>{t("approvalRule")}</span><time>PENDING APPROVAL</time></div>{approvals.error?<ErrorPanel error={approvals.error}/>:<section className="panel"><PanelTitle title={t("approvals")} tag={`${approvals.data?.items.length??0} PENDING`}/>{approvals.data?.items.length?<div className="event-list">{approvals.data.items.map((task)=><div key={task.id}><StatusPill status={task.status}/><b>{task.id.slice(0,8).toUpperCase()}</b><span>{task.account_id} · {task.requested_amount}</span><button className="primary-button" onClick={()=>approve.mutate(task.id)} disabled={approve.isPending}>APPROVE</button><button className="alarm-button" onClick={()=>reject.mutate(task.id)} disabled={reject.isPending}>REJECT</button></div>)}</div>:<EmptyState text={t("noPendingApprovals")}/>}</section>}</>;
}
export function AccountsPage(){const {t}=useTranslation();const accounts=useQuery({queryKey:["accounts"],queryFn:api.accounts});if(accounts.error)return <><PageHeader eyebrow="PRIVATE / RECONCILIATION" title={t("accountsReconciliation")}/><ErrorPanel error={accounts.error}/></>;return <><PageHeader eyebrow="PRIVATE / RECONCILIATION" title={t("accountsReconciliation")} description={t("privatePlaneDescription")}/><section className="panel"><PanelTitle title={t("accountsReconciliation")} tag={`${accounts.data?.length??0} ACCOUNTS`}/>{accounts.data?.length?<div className="event-list">{accounts.data.map((account)=><div key={account.id}><StatusPill status={account.enabled?"ready":"locked"}/><b>{account.label}</b><span>{account.exchange} · {account.environment}</span><code>{account.latest_reconciliation?.status??"not_reconciled"}</code></div>)}</div>:<EmptyState text={t("noAccountsConfigured")}/>}</section></>}
export function RiskPage(){const {t}=useTranslation();const risk=useQuery({queryKey:["risk"],queryFn:api.risk});if(risk.error)return <><PageHeader eyebrow="RISK / CONTROL PLANE" title={t("riskControls")}/><ErrorPanel error={risk.error}/></>;return <><PageHeader eyebrow="RISK / CONTROL PLANE" title={t("riskControls")} description={t("privatePlaneDescription")}/><section className="metric-strip"><Metric label="RISK LIMITS" value={String(risk.data?.limits.length??0)}/><Metric label="KILL SWITCHES" value={String(risk.data?.kill_switches.length??0)}/><Metric label="DECISIONS" value={String(risk.data?.decisions.length??0)}/></section><section className="panel"><PanelTitle title={t("riskControls")} tag="ZERO DEFAULT"/><p>{t("zeroDefaultRisk")} · {t("killSwitchHierarchy")}</p><pre>{JSON.stringify(risk.data?.kill_switches??[],null,2)}</pre></section></>}
