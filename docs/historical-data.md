...
逐笔成交以毫秒时间戳分页。下一页会包含上一页页尾所在的毫秒，并依靠交易所成交 ID 幂等去重；不会把游标强行推进到 `+1ms`。如果交易所在同一毫秒返回满页且没有可安全使用的通用次级游标，下一页将无法推进，Ballast 以 `historical_cursor_not_advanced` 明确失败并保留此前已提交进度，避免静默漏掉成交。交易所明确返回 `exhausted` 时，即使非空短页的最后成交早于请求终点，完成游标也记录为任务 `end_at`，表示该区间已经扫描完毕；未返回 `exhausted` 的任务不会获得这一覆盖证明。
