...
回放绑定不可变策略模板版本，要求完整且已完成的历史成交补数覆盖，并使用冻结的成交 ingestion 快照运行 `trade_vwap_proxy`。运行响应包含 `instrument_snapshot`、`data_snapshot` 与 `coverage_snapshot`；覆盖错误、幂等行为、数量单位和 OKX TWAP/POV 对比示例见 `docs/replay.md`。
