...
| 交易所 | ccxt ID | 现货 | 线性永续 | 反向永续 | REST 盘口 | WS 盘口 | WS 成交 | 历史 OHLCV | 历史成交 | 沙盒下单 |
|---|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|
| Binance | `binance` | 是 | 是 | 是 | 已接入 | 已接入 | 已接入 | 已接入 | 已接入 | 后续验证 |
| OKX | `okx` | 是 | 是 | 是 | 已接入 | 已接入 | 已接入 | 已接入 | 已接入 | 后续验证 |
| Bybit | `bybit` | 是 | 是 | 是 | 已接入 | 已接入 | 已接入 | 已接入 | 已接入 | 后续验证 |
| Gate.io | `gate` | 是 | 是 | 按公开市场实际结果 | 已接入 | 已接入 | 已接入 | 已接入 | 已接入 | 后续验证 |
| Bitget | `bitget` | 是 | 是 | 按公开市场实际结果 | 已接入 | 已接入 | 已接入 | 已接入 | 已接入 | ccxt 当前未声明 |

“已接入”表示统一适配代码与 fixture 测试完成，不表示当前开发机已完成公网验证。`ListInstruments` 以交易所实际返回为准，不合成不存在的市场。

历史能力和回溯/保留限制详见 `docs/historical-data.md`。服务运行时读取 ccxt 的明确能力标志；不支持时失败，不静默换交易所或数据类型。
...
| 交易所 | 账户快照 | 按 `client_order_id` 查询 | 下单 / 取消 / 私有流 |
|---|---|---|---|
| OKX | 已接入，只读 | 已接入，只读 | 禁用 |
| Binance / Bybit / Gate.io / Bitget | 未接入 | 未接入 | 禁用 |

OKX 查询使用 ccxt `fetchOrder` 的 `clientOrderId` 参数映射到 `clOrdId`，未知原始状态显式失败，不根据相似状态猜测。该能力用于收敛 `submission_unknown` 和本地非终态订单，不代表真实下单链路已开放。
