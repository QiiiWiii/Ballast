...
- [x] `CHANGELOG` 收敛为 `0.1.0` 说明；去掉与代码不符的表述
- [x] `git tag v0.1.0` 触发 Docker Hub 多架构镜像（`0.1.0` / `0.1` / `v0.1.0` / `latest` / `sha-*`）；后续补丁沿用同一发布轨道
...
- [x] 五家交易所可使用统一公开 OHLCV 窗口运行 TWAP/POV 验证矩阵，并保留数据覆盖差异
